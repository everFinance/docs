...
1. 通过 [everpay info API](#TODO:) 获取 everPay 在 Ethereum 的多签合约地址 `ethLocker`
2. 通过 imToken / MetaMask 等区块链钱包 工具，给该 `ethLocker` 多签合约地址转账想要充值的资产与数量
3. 等待 该笔交易 6 个区块确认
4. 通过 [everpay balances API](#TODO:) 获取此钱包对应的 everPay 账户，在everPay 上的资产信息
...
## Arweave 账户模型
### arConnect 等区块链钱包
1. 通过 [everpay info API](#TODO:) 获取 everPay 在 Arweave 上门限签名管理的钱包地址 `arLocker`
