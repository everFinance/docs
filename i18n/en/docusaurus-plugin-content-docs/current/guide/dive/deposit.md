...
## everPay transaction
The everPay transaction for recharging is generated by the everPay backend service after detecting the receipt of assets at the everPay DAO managed wallet address. [`Schema`](./transaction#schema) first generated, then everPay will use the **everPay signature wallet** to sign it and submit it to everPay server to mark the addition of assets to user's account.
